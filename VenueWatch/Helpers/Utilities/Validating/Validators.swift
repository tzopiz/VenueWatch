...
extension Utilities.Validators {
    ...
    private static func isValidPassword(_ password: String?) -> ValidationResult<String, String> {
        guard let password = password, password.contains(where: { $0.isUppercase }) else {
            return .invalid(App.string.noUppercaseLetter())
        }
        
        guard password.contains(where: { $0.isLowercase }) else {
            return .invalid(App.string.noLowercaseLetter())
        }
        
        guard password.contains(where: { $0.isNumber }) else {
            return .invalid(App.string.noDigit())
        }
        
        let specialCharacters = CharacterSet(charactersIn: "!?@#$%^&*()-_")
        guard password.rangeOfCharacter(from: specialCharacters) != nil else {
            return .invalid(App.string.noSpecialCharacter())
            
        }
        guard password.count >= 8 && password.count <= 64 else {
            return .invalid(App.string.insufficientLength())
        }
    ...
    private static func isValidUsername(_ username: String?) -> ValidationResult<String, String> {
        ...
        guard username.range(
            ...
            return .invalid("Invalid username format")
        }
